...
<body class="bg-[#f1f5f9] text-slate-900 font-sans">
    ...
    <nav class="bg-white border-b border-slate-200 py-4 px-8 flex justify-between items-center sticky top-0 z-50 shadow-sm">
        ...
        <div class="flex items-center gap-6">
            <a href="<?php echo e(route('forum.index')); ?>" class="text-slate-700 hover:text-blue-700 font-bold transition flex items-center gap-2">
    ...
    <main class="max-w-7xl mx-auto py-10 px-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/60 border border-slate-100 overflow-hidden relative">
                    ...
                    <div class="relative -mt-12 text-center">
                        <div class="relative inline-block group cursor-pointer">
                            <img src="<?php echo e($user->avatar ? asset('storage/'.$user->avatar) : 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&background=0D8ABC&color=fff'); ?>"
                            ...
                            <form id="avatar-form" action="<?php echo e(route('user.profile.update')); ?>" method="POST" enctype="multipart/form-data">
                                <?php echo csrf_field(); ?>
                        ...
                        <h2 class="mt-3 text-xl font-black text-slate-800"><?php echo e($user->name); ?></h2>
                        <p class="text-blue-600 font-bold text-xs uppercase tracking-[0.2em] text-center mb-4">
                            Étudiant Membre — <span class="bg-blue-100 px-2 py-0.5 rounded text-blue-700"><?php echo e($user->student_level ?? 'L3'); ?></span>
                        </p>
                    </div>
            ...
            <div class="lg:col-span-8 space-y-8" x-data="{ search: '' }">
                ...
                <!-- NOUVEAUTÉS -->
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(isset($nouveautes) && count($nouveautes) > 0): ?>
                <div class="bg-gradient-to-br from-blue-600 to-indigo-900 rounded-[3rem] p-8 shadow-xl text-white relative overflow-hidden group">
                    ...
                    <h3 class="text-xs font-black uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
                        ...
                        Nouveautés pour vous (<?php echo e($user->student_level); ?>)
                    </h3>
                    <div class="space-y-4">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $nouveautes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nouv): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="bg-white/10 backdrop-blur-md border border-white/10 p-5 rounded-3xl flex justify-between items-center hover:bg-white/20 transition-all"
                                 x-show="search === '' || '<?php echo e(strtolower($nouv->titre)); ?>'.includes(search.toLowerCase())">
                                <div class="flex items-center gap-5">
                                    <div class="w-12 h-12 bg-white text-blue-600 rounded-2xl flex items-center justify-center shadow-lg"><i class="fas <?php echo e($nouv->type == 'Vidéo' ? 'fa-play' : 'fa-file-pdf'); ?>"></i></div>
                                    <div>
                                        <p class="font-bold text-base leading-tight"><?php echo e($nouv->titre); ?>

                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($nouv->is_premium): ?><span class="ml-2 text-[9px] bg-yellow-400 text-slate-900 px-2 py-0.5 rounded-full font-black uppercase"><?php echo e($nouv->price); ?> CFA</span><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                        </p>
                                        <p class="text-[10px] opacity-60 uppercase font-black tracking-widest mt-1"><?php echo e($nouv->matiere->nom); ?></p>
                                    </div>
                                </div>
                                <a href="<?php echo e(route('ressource.download', $nouv->id)); ?>" target="_blank" class="bg-white text-blue-600 w-10 h-10 rounded-full flex items-center justify-center hover:scale-110 transition shadow-lg"><i class="fas fa-arrow-right"></i></a>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <form action="<?php echo e(route('user.history.clear')); ?>" method="POST"><?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                            <button class="text-[10px] font-bold text-red-400 hover:text-red-600 bg-red-50 px-3 py-1 rounded-full uppercase">Vider</button>
                        </form>
                    </div>
                    <div class="space-y-3">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $downloads; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $download): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="flex items-center justify-between group p-4 hover:bg-blue-50/50 rounded-2xl transition border border-transparent hover:border-blue-100" x-show="search === '' || '<?php echo e(strtolower($download->titre)); ?>'.includes(search.toLowerCase())">
                                <div class="flex items-center gap-5">
                                    <div class="w-12 h-12 bg-slate-100 group-hover:bg-white text-slate-500 group-hover:text-blue-600 rounded-xl flex items-center justify-center text-lg shadow-sm"><i class="fas <?php echo e($download->type == 'Vidéo' ? 'fa-play' : 'fa-file-pdf'); ?>"></i></div>
                                    <div>
                                        <a href="<?php echo e(route('ressource.download', $download->id)); ?>" target="_blank" class="font-black text-slate-800 hover:text-blue-600 block text-base"><?php echo e($download->titre); ?></a>
                                        <span class="text-[10px] text-slate-400 uppercase font-bold"><?php echo e($download->matiere->nom); ?></span>
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($download->is_premium): ?><span class="ml-2 text-[8px] bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-black uppercase border border-amber-200">⭐ Débloqué</span><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    </div>
                                </div>
                                <form action="<?php echo e(route('user.history.destroy', $download->id)); ?>" method="POST"><?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                ...
                <div class="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-[2rem] p-8 shadow-sm border border-indigo-200">
                    ...
                    <form action="<?php echo e(route('user.memoire.store')); ?>" method="POST" enctype="multipart/form-data" class="space-y-5">
                        <?php echo csrf_field(); ?>
                        <div>
                ...
                <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
                    ...
                    <div class="space-y-3">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $mesMemoires; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $memoire): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="flex items-start justify-between group p-5 hover:bg-indigo-50/50 rounded-2xl transition border border-indigo-100 bg-indigo-50/30">
                                <div class="flex-1">
                                    <a href="<?php echo e(route('ressource.download', $memoire->id)); ?>" target="_blank" class="font-black text-slate-800 hover:text-indigo-600 block text-base"><?php echo e($memoire->titre); ?></a>
                                    <p class="text-[10px] text-slate-400 uppercase font-bold mt-2">Publié le <?php echo e($memoire->created_at->format('d/m/Y à H:i')); ?></p>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($memoire->description): ?>
                                        <p class="text-xs text-slate-600 mt-2 line-clamp-2"><?php echo e($memoire->description); ?></p>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </div>
                                <form action="<?php echo e(route('user.memoire.destroy', $memoire->id)); ?>" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce mémoire ?');" class="ml-4">
                                    <?php echo csrf_field(); ?>
                ...
                <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    <div class="divide-y divide-slate-100">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $mesSujets; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sujet): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="p-6 flex justify-between items-center hover:bg-slate-50 transition" x-show="search === '' || '<?php echo e(strtolower($sujet->titre)); ?>'.includes(search.toLowerCase())">
                                <div><a href="<?php echo e(route('forum.sujet', $sujet->id)); ?>" class="font-bold text-slate-800 hover:text-blue-600 text-lg block"><?php echo e($sujet->titre); ?></a><p class="text-xs text-slate-400 mt-1"><?php echo e($sujet->created_at->diffForHumans()); ?></p></div>
                                <form action="<?php echo e(route('user.sujet.destroy', $sujet->id)); ?>" method="POST"><?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
...
<?php /**PATH C:\Users\Mamad\Documents\Dossier Mio-Ressource\mio_ressources\resources\views/user/dashboard.blade.php ENDPATH**/ ?>